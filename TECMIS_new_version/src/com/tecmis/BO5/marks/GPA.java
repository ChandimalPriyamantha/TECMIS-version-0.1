...
public class GPA {
    ...
    private int type;
    private int gpa_value;

    public int getGpa_value() {
    return gpa_value;
   
    }
    
     public void setGpa_value(int gpa_value) {
    ...
    public List<GPA> list(String sid) {
       ...
       List<GPA> list = new ArrayList<GPA>();
        try {
            ...
            while(rs.next()){
                ...
                GPA gpa = new GPA();
                gpa.setStudentID(rs.getString("student_id"));
                gpa.setDepartmentID(rs.getString("student_department_department_id"));
                gpa.setCourseID(rs.getString("course_id"));
                gpa.setLevel(rs.getString("level"));
                gpa.setSemester(rs.getString("semester"));
                gpa.setQuize_01(rs.getInt("Quiz_01"));
                gpa.setQuize_02(rs.getInt("Quiz_02"));
                gpa.setQuize_03(rs.getInt("Quiz_03"));
               gpa.setQuize_04(rs.getInt("Quiz_04"));
                gpa.setMidterm(rs.getInt("mid_term"));
                gpa.setAssesment_01(rs.getInt("Assignment_01"));
                gpa.setAssesment_02(rs.getInt("Assignment_02"));
                gpa.setFinalPracticle(rs.getInt("final_practical"));
                gpa.setFinalTheory(rs.getInt("final_theory"));
                gpa.setType(rs.getInt("Type"));
                gpa.setGpa_value(rs.getInt("gpa_value"));
    ...
    public GPA get(String sid) {
         GPA gpa = new GPA();
        try {
            
            Connection con = TecmisDB.getConnection();
            String sql = "SELECT * FROM mark WHERE student_id=?";
            //String sql = "SELECT * FROM mark  INNER JOIN student ON mark.student_id = student.id where student.id=?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, sid);
